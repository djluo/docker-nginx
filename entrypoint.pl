...
system("/usr/sbin/userdel",  "nginx");
#system("/usr/sbin/groupdel", "nginx");
unless (getpwuid("$uid")){
  add_user("nginx",  "$uid");
}

system("mkdir", "/etc/nginx/vhost.d") unless ( -d "/etc/nginx/vhost.d" );

system("mkdir", "/var/log/nginx") unless ( -d "/var/log/nginx" );
system("chown nginx -R /var/log/nginx");
system("chown nginx -R /var/cache/nginx");
